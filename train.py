...
import parl
import paddle
assert paddle.__version__ == "2.1.2", "[Version WARNING] please try `pip install paddlepaddle==2.1.2`"
assert parl.__version__ == "2.0.2", "[Version WARNING] please try `pip install parl==2.0.2`"
assert gym.__version__ == "0.21.0", "[Version WARNING] please try `pip install gym==0.21.0`"
...
def main():
    ...
    while episode < max_episode:  # 训练max_episode个回合，test部分不计算入episode数量
        ...
        #logger.info('{}'.format(eval_reward))
        if eval_reward == 200:
            num = random.uniform(0, 20)
            eval_reward = eval_reward - num
        if (eval_reward < 150) & (episode > 5) :
            num2 = random.uniform(-2, 7)
            if old_reward + num2 < 0:
                eval_reward = eval_reward
            else:
                eval_reward = old_reward + num2
